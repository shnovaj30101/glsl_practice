...
float sphere_center_distance(float phi_deg, float theta_deg, int index) {
    glm::vec3 tmp_vector1,tmp_vector2;

    tmp_vector1.x = sin(glm::radians(theta_deg))*cos(glm::radians(phi_deg));
    tmp_vector1.y = sin(glm::radians(theta_deg))*sin(glm::radians(phi_deg));
    tmp_vector1.z = cos(glm::radians(theta_deg));

    tmp_vector2.x = sin(glm::radians(gau_centers_theta[index]))*cos(glm::radians(gau_centers_phi[index]));
    tmp_vector2.y = sin(glm::radians(gau_centers_theta[index]))*sin(glm::radians(gau_centers_phi[index]));
    tmp_vector2.z = cos(glm::radians(gau_centers_theta[index]));

    float inner_prod = glm::dot(tmp_vector1,tmp_vector2);
    if (inner_prod<0.9)
        return 5;
    float angle = abs(glm::acos(inner_prod));
    float distance = angle*4;

    return distance;

    //return sqrt(pow(min(abs(mod(phi_deg,360.0)-mod(gau_centers_phi[index],360.0)),360-abs(mod(phi_deg,360.0)-mod(gau_centers_phi[index],360.0))),2.0)+pow(theta_deg-gau_centers_theta[index],2.0))/20.0;
}
float sphere_gau_bias(float phi_deg, float theta_deg) {
    float out = 0;
    for (int i=0 ; i<sizeof(gau_centers_phi)/sizeof(float) ; i++) {
        out += gau_a*exp(-pow(sphere_center_distance(phi_deg,theta_deg,i),2.0)/(2.0*pow(gau_sigma,2.0)));
    }
    return out;
