...
int main()
{
    ...
    unsigned int VBO, VAO, EBO; glGenVertexArrays(1, &VAO);
    glGenBuffers(1, &VBO);
    glGenBuffers(1, &EBO);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        for (int i=0 ; i<10 ; i++) {
            // render container
            glm::mat4 model2;
            model2 = glm::translate(model2,cube_pos[i]);
            model[i]= glm::rotate(model[i], (float)glm::radians(sin(glfwGetTime()))*(i+1), glm::vec3(1.0f, 0.0f, 0.0f));
            model[i]= glm::rotate(model[i], (float)glm::radians(sin(glfwGetTime()))*(i+1), glm::vec3(0.0f, 1.0f, 0.0f));
            ourShader.setMat4("Model", model2*model[i]);
